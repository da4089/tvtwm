...
int height;
{
	...
	dwidth *= Scr->PannerScale;
	dheight *= Scr->PannerScale;
    }
    ...
    Bool sendEvent;			/* whether or not to force a send */
{
    ...
    if (tmp_win->w == Scr->Panner)
	ResizeDesktop(w, h-tmp_win->title_height);
	...
	int width, height;
	width = tmp_win->frame_width / Scr->PannerScale;
	height = tmp_win->frame_height / Scr->PannerScale;
	if (width <= 0)
	    width = 1;
	if (height <= 0)
	    height = 1;
	XMoveResizeWindow(dpy, tmp_win->virtualWindow,
    ...
    if (sendEvent)
	SendSyntheticConfigureNotify(tmp_win);
}
...
TwmWindow *tmp_win;
{
    ...
    client_event.xconfigure.x = (tmp_win->frame_x + tmp_win->frame_bw - tmp_win->old_bw);
    client_event.xconfigure.y = (tmp_win->frame_y + tmp_win->frame_bw +
				 tmp_win->title_height - tmp_win->old_bw);
    if (Scr->VirtualDesktop && !tmp_win->sticky) {
	client_event.xconfigure.x -= Scr->vdtPositionX;
	client_event.xconfigure.y -= Scr->vdtPositionY;
    }
