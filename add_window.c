...
IconMgr *iconp;
{
    ...
    int namelen;
    int bw2;
    int cancel;				/* cancel flag for window move */
    int tmpX;				/* temp variable */
    int saveDelta;			/* save the Scr->MoveDelta */
	...
	getTWM_FLAGS(tmp_win->w, &tmp_win->flags);

    if (Scr->VirtualDesktop) {
	tmp_win->sticky = 
	    (short)LookInList(Scr->StickyL, tmp_win->full_name, &tmp_win->class);
	if (tmp_win->w == Scr->Panner)
	    tmp_win->sticky = True;

	if (RestartPreviousState) {
	    if (tmp_win->flags & TWM_FLAGS_STICKY)
		tmp_win->sticky = True;
	}
    }

    /* set flags for twm restart */
	...
	tmp_win->virtualWindow = MakeVirtual(tmp_win,
	    ...
	    tmp_win->title.back, tmp_win->border);
    }
    else
	tmp_win->root = Scr->Root;

    tmp_win->frame = XCreateWindow (dpy, tmp_win->root, 0, 0,
	...
	if (tmp_win->root == Scr->VirtualDesktop) {
	    tmp_win->attr.x += Scr->vdtPositionX;
	    tmp_win->attr.y += Scr->vdtPositionY;
	}
      } else {				/* else prompt */
	      ...
	      tmp_win->wmhints->initial_state == IconicState))
	{
		 ...
		 */
		stat = XGrabPointer(dpy, Scr->Root, True,
		...
		&AddingX, &AddingY, &JunkX, &JunkY, &JunkMask);
	    if (tmp_win->root == Scr->VirtualDesktop)
		XMoveWindow(dpy, tmp_win->frame, AddingX+Scr->vdtPositionX,
		    AddingY+Scr->vdtPositionY);
	    else
		XMoveWindow(dpy, tmp_win->frame, AddingX, AddingY);

	    saveDelta = Scr->MoveDelta;
	    Scr->MoveDelta = 0;
	    StartMove(tmp_win, tmp_win->frame, tmp_win->title_height,
		&AddingX, &AddingY, &cancel, OUT_PANNER, 1, 0, 0, True, False);
	    Scr->MoveDelta = saveDelta;
...
#undef HALF_AVE_CURSOR_SIZE
    ...
    /* Possibly adjust the position of the window if PPosition is
     * being used.  In our case, if PPosition is set, we will translate
     * the coordinates of the window into virtual desktop coordinates
     */
    if (tmp_win->root == Scr->VirtualDesktop &&
    ...
    /* tell the window what his root window id is */
    /* this MUST com before SetupFrame so the synthetic ConfigureNotify
     * event follows this property
     */
    SetSWM_ROOT(tmp_win);

    tmpX = tmp_win->frame_x;
    tmp_win->frame_x = -tmpX;	/* this will case a synthetice ConfigureNotify event */
    SetupFrame (tmp_win, tmpX, tmp_win->frame_y,
    ...
    unsigned long *flags;
{
    ...
    if (XGetWindowProperty (dpy, w, _XA_TWM_FLAGS, 0L, 1L, False, _XA_TWM_FLAGS,
...
TwmWindow *tmp_win;
{
    ...
    if (tmp_win->sticky)
	data |= TWM_FLAGS_STICKY;


    XChangeProperty (dpy, tmp_win->w, _XA_TWM_FLAGS, _XA_TWM_FLAGS, 32, 
